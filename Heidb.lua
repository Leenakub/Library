...
local FileName = tostring(game.Players.LocalPlayer.UserId).."_Settings-"..game:GetService("RbxAnalyticsService"):GetClientId()
local BaseFolder = "AFKHUB"
local SubFolder = "Fisch-"..tostring(game.PlaceId)

function SaveSetting()
local json
if writefile then
    json = game:GetService("HttpService"):JSONEncode(getgenv().Settings)
...
do
                    ...
                    if SafeMode.Value == false or UseZone.Value == false then
                            ...
                            if character[getgenv().Settings.Rod].values.casted.Value == false and Casted == false then
                                pcall(function()
                                    ...
                                    wait(.1)
                                    character[getgenv().Settings.Rod].events.cast:FireServer(100)
                                    Casted = true
                                    wait(1)
                                end)
                                    ...
                                    pcall(function()
                                        ...
                                        if shakeui then
                                            for _ = 1,2 do
                                                ...
                                                game:GetService('VirtualInputManager'):SendMouseButtonEvent(X + XS, Y + YS, 0, false, Button, 1)
                                                game:GetService('VirtualInputManager'):SendMouseButtonEvent(X + XS, Y + YS, 0, true, Button, 1)
                                                game:GetService('VirtualInputManager'):SendMouseButtonEvent(X + XS, Y + YS, 0, false, Button, 1)
                                                game:GetService('VirtualInputManager'):SendMouseButtonEvent(X + XS, Y + YS, 0, true, Button, 1)
                                            end
                                        end
                            ...
                            if character[getgenv().Settings.Rod].values.bite.Value == true and character[getgenv().Settings.Rod].values.casted.Value == true and character[getgenv().Settings.Rod]:FindFirstChild("bobber") and character[getgenv().Settings.Rod].values.bobberzone.Value ~= "" then
                                pcall(function()
                                    if game:GetService("Players").LocalPlayer.PlayerGui.reel.bar:FindFirstChild("playerbar") then
                                        if game:GetService("Players").LocalPlayer.PlayerGui.reel.bar.playerbar.Size ~= UDim2.new(1, 0, 1, 0) then
                    ...
                    for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.hud.safezone.backpack:GetDescendants()) do
                        if v.Name == "bait" and v:IsA("TextLabel") then
                            ...
                            elseif getgenv().Settings.Bait == "None" then
                                game:GetService("Players").LocalPlayer.PlayerGui.hud.safezone.equipment.bait.scroll.safezone.e:FireServer("None")
                                BaitEquiped = true
                            end
                        end
                    end
    ...
    WalkOnWater:OnChanged(function()
        ...
        if not workspace:FindFirstChild("WATERWALKPART") then
            ...
            float.Size = Vector3.new(25, 2.5, 25)
        end
        local waterLevelY = 127.5
            while WalkOnWater.Value do
                wait()
                local player = humanoidRootPart.Position
                for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
                    if v.Name == "WATERWALKPART" and v:IsA("BasePart") and v.CFrame ~= humanoidRootPart.CFrame + Vector3.new(player.X, waterLevelY, player.Z) then
                        v.CFrame = humanoidRootPart.CFrame + Vector3.new(player.X, waterLevelY, player.Z)
                    end
                end
            end
        end)
    ...
    while wait(0.1) do
        if WalkOnWater.Value == false and workspace:FindFirstChild("WATERWALKPART") then
            game.workspace.WATERWALKPART:Destoy()
        end
    end
